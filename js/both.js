/**
 * Antika Gallery — Guide Bot (Secure Version)
 * الـ API Key محمي على السيرفر — مش موجود هنا خالص
...
(function () {
  ...
  // ══ CONFIG ════════════════════════════════════════════════════
  // بس الـ Supabase URL هنا — مفيش API keys خالص
  const PROXY_URL   = 'https://tutcepymwnjvbbmdmsjm.supabase.co/functions/v1/grok-proxy';
  ...
  const CSS = `
    ...
    #_abot-close {
      ...
      background:none;color:#7A6A54;cursor:pointer;
    ...
    #_abot-inp { flex:1;background:#252017;border:1.5px solid #3A3020;border-radius:14px;padding:10px 14px;color:#F0EAD8;font-family:'Cairo',sans-serif;font-size:14px;resize:none;outline:none;min-height:42px;max-height:100px;line-height:1.5;transition:border-color .2s }
    #_abot-inp:focus { border-color:#C9A84C }
    #_abot-inp::placeholder { color:#7A6A54 }
    #_abot-send { width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,#C9A84C,#A07830);border:none;color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:transform .2s,opacity .2s }
  ...
  document.body.insertAdjacentHTML('beforeend', `
    ...
    <div id="_abot-win" role="dialog" aria-label="مساعد أنتيكا">
      ...
      <div id="_abot-inp-area">
        <textarea id="_abot-inp" placeholder="اكتب سؤالك هنا..." rows="1"
          onkeydown="_abotKey(event)" oninput="_abotResize(this)" maxlength="500"></textarea>
  ...
  function _abotWelcome() {
    _abotAppend('bot', `أهلاً بيكِ في أنتيكا جاليري! 🌟<br><br>أنا مساعدتك هنا — أقدر أساعدك في:<ul><li>كيفية الشراء وإتمام الطلب</li><li>تتبع طلبك الحالي</li><li>طرق الدفع المتاحة</li><li>أي استفسار عن الموقع</li></ul>اسأليني أي حاجة! 💛`);
  }
  ...
  function _abotShowTyping() {
    ...
    const row  = document.createElement('div');
    row.className = '_abot-row'; row.id = id;
    row.innerHTML = `
      <div class="_abot-mav b">✦</div>
      <div class="_abot-typing">
        <div class="_abot-tdot"></div><div class="_abot-tdot"></div><div class="_abot-tdot"></div>
      </div>`;
    msgs.appendChild(row);
    msgs.scrollTop = msgs.scrollHeight;
    return id;
  ...
  function _abotRemoveTyping(id) { document.getElementById(id)?.remove(); }
